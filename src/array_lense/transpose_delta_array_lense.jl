...
function (τLp::τArrayLensePlan{2,n,Tf,d})(ẏ, t, y) where {n,Tf,d}
	...
	for i = 1:n
		...
		## --- alt option (replaced the inbounds with turbo)
		@inbounds @. τLp.w[1] += τf * (τLp.mm[1,1] * τLp.∇y[1] + τLp.mm[2,1] * τLp.∇y[2])  
		@inbounds @. τLp.w[2] += τf * (τLp.mm[1,2] * τLp.∇y[1] + τLp.mm[2,2] * τLp.∇y[2]) 
		## --- alt option
	...
	## --- (replaced the inbounds with turbo)
	@inbounds @. ẏ[1] =  - τLp.w[1] - t * τLp.mm[1,1]
	@inbounds @. ẏ[2] =  - τLp.w[2] - t * τLp.mm[2,1]
	## --- alt option
