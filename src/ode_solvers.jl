...
function odesolve_RK38(f!, y₀::AbstractArray, t₀, t₁, nsteps)
    ...
    for t in range(t₀,t₁,length=nsteps+1)[1:end-1]
        f!(v₁, t, y)
        f!(v₂, t + h⅓, (@inbounds @. y′ = y + h⅓*v₁))
        f!(v₃, t + h⅔, (@inbounds @. y′ = y - h⅓*v₁ + h*v₂))
        f!(v₄, t + h,  (@inbounds @. y′ = y + h*v₁  - h*v₂ + h*v₃))
        @inbounds @. y += h*(v₁ + 3v₂ + 3v₃ + v₄)/8
    end
    return y
end
...
function odesolve_RK38(f!, y₀::NTuple{m}, t₀, t₁, nsteps) where {m}
    ...
    for t in range(t₀,t₁,length=nsteps+1)[1:end-1]
        ...
        for i=1:m; (@inbounds @. y′[i] = y[i] - h⅓*v₁[i] + h*v₂[i]); end
        f!(v₃, t + h⅔,  y′)

        for i=1:m; (@inbounds @. y′[i] = y[i] + h*v₁[i]  - h*v₂[i] + h*v₃[i]); end
        f!(v₄, t + h,  y′)

        for i=1:m; (@inbounds @. y[i] += h*(v₁[i] + 3v₂[i] + 3v₃[i] + v₄[i])/8); end
    end
    return y
end
